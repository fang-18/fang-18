...
var gameConfig = {
    ...
    cols: 3, //列数
    isOver: false, //游戏是否结束
    imgurl: "img/02.jpg", //图片路径，注意：相对的是页面路径
...
function init() {
    ...
    var imgs = document.querySelector('.imgs img');
    var end = document.querySelector('.end');
    document.querySelector('.again').onclick = function () { shuffle(); }
    var log = document.querySelector('.check').onclick = function () {
        if (imgs.style.display == "none") {
            gameConfig.dom.style.display = "none";
            imgs.style.display = "block";
        } else {
            imgs.style.display = "none";
            gameConfig.dom.style.display = "block";
        }
    }
    //让图片隐藏和现实
    log();
